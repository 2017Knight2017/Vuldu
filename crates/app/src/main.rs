...
use crate::{
	...
	graphics::{GraphicsContext, GraphicsFlags},
};
use clap::Parser;
use engine::*;
...
impl ApplicationHandler for App {
	fn resumed(&mut self, event_loop: &ActiveEventLoop) {
		...
		let handles = match (display_handle, window_handle) {
			(RawDisplayHandle::Wayland(d), RawWindowHandle::Wayland(w)) => WindowHandles {
				display_ptr: d.display.as_ptr() as usize,
				window_ptr: w.surface.as_ptr() as usize,
				is_x11: false,
			},
			(RawDisplayHandle::Xlib(d), RawWindowHandle::Xlib(w)) => WindowHandles {
				display_ptr: d.display.unwrap().as_ptr() as usize,
				window_ptr: w.window as usize,
				is_x11: true,
			},
			(RawDisplayHandle::Windows(_), RawWindowHandle::Win32(w)) => WindowHandles {
			...
			(_, _) => {
				eprintln!(
					"Unsupported platform. Available platforms are: Linux Wayland, Linux X11, Windows, MacOS."
				);
				event_loop.exit();
				return;
			}
		...
		match event {
			...
			WindowEvent::KeyboardInput { event, .. } => {
				...
				if self.last_buttons_pressed[0] != Some(event.physical_key) {
					...
					self.last_buttons_pressed
						.push_front(Some(event.physical_key));
				}

				match event.physical_key {
					PhysicalKey::Code(KeyCode::KeyW) => {
						self.current_input.move_forward = is_pressed
					}
					PhysicalKey::Code(KeyCode::KeyS) => {
						self.current_input.move_backward = is_pressed
					}
					...
					PhysicalKey::Code(KeyCode::ShiftLeft) => {
						self.current_input.move_down = is_pressed
					}
					PhysicalKey::Code(KeyCode::Digit1) => {
						self.current_input.switch_fist_chainsaw = is_pressed
					}
					PhysicalKey::Code(KeyCode::Digit2) => {
						self.current_input.choose_pistol = is_pressed
					}
					PhysicalKey::Code(KeyCode::Digit3) => {
						self.current_input.choose_shotgun = is_pressed
					}
					PhysicalKey::Code(KeyCode::Digit4) => {
						self.current_input.choose_chaingun = is_pressed
					}
					PhysicalKey::Code(KeyCode::Digit5) => {
						self.current_input.choose_rlauncher = is_pressed
					}
					PhysicalKey::Code(KeyCode::Digit6) => {
						self.current_input.choose_plasma = is_pressed
					}
