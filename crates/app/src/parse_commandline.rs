...
const BYTE_SHADOWS_HELP: &str =
	"Make shadows submit values of light in 0..255 range (default range is 0..32).";
const FAST_MONSTERS_HELP: &str = "Currently does nothing.";
const COOP_HELP: &str =
	"Turn on the Co-op mode. Currently just spawns additional objects on a level.";
const DEATHMATCH_HELP: &str =
	"Turn on the Deathmatch mode. Currently just prevents keys from spawning on a level.";
