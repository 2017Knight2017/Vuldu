...
use rodio::Source;
use rodio::{MixerDeviceSink, SpatialPlayer, buffer::SamplesBuffer, math::nz};
...
impl DoomSfxPlayer {
	...
	fn play_head_sound(&mut self, src: SamplesBuffer) {
		...
		self.spatial_players[8].append(src.amplify(0.6));
	}
}
		...
		for event in self.buffer.drain(..) {
			...
			match event.pos {
				...
				Some(emitter_pos) => {
					...
					if dy_m.abs() < METERS_DIST_CAP {
						dy_m = METERS_DIST_CAP * dy_m.signum();
					}
					if dz_m.abs() < METERS_DIST_CAP {
						dz_m = METERS_DIST_CAP * dz_m.signum();
					}
					...
					let volume_factor =
						VOLUME_NEAR + approx_dist / MAX_AUDIBLE_DIST * (VOLUME_FAR - VOLUME_NEAR);
