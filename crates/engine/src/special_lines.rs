use crate::{Button, MobjNum, MobjType, PlayerInventory, SfxEvent, p_change_switch_texture};
use wad_parser::{Level, Line, LineFlags, LineId};
...
pub fn p_use_special_line(
	...
	match line.special {
		...
		| 20 | 21 | 23 | 29 | 41 | 71
		| 49 | 50 | 51 | 55 | 101 | 102
		| 103 | 111 | 112 | 113 | 122 | 127
		| 131 | 133 | 135 | 137 | 140 
		if let Some(y) = player_y_opt && effect(line, inv_opt) => {
			p_change_switch_texture(level, line_id, false, audio, buttons, y);
			level.state.lines[line_id.0].used = true;
		}

		42 | 43 | 45 | 60 | 61 | 62
		| 63 | 64 | 66 | 67 | 65 | 68
		| 69 | 70 | 114 | 115 | 116 | 123
		| 132 | 99 | 134 | 136 | 138 | 139
		if let Some(y) = player_y_opt && effect(line, inv_opt) => {
			p_change_switch_texture(level, line_id, true, audio, buttons, y);
		}
