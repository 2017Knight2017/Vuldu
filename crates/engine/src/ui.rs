...
pub fn update_keys_ui(cards: [bool; NUMCARDS], ui_to_render: &mut Vec<(Ui, f32, f32)>) {
	...
	if cards[Card::BlueSkull as usize] {
		ui_to_render.push((Ui::STKEYS3, keys_x_offset, STBAR_Y_OFFSET + 3.1));
	} else if cards[Card::BlueCard as usize] {
		ui_to_render.push((Ui::STKEYS0, keys_x_offset, STBAR_Y_OFFSET + 4.0));
	}

	if cards[Card::YellowSkull as usize] {
		ui_to_render.push((Ui::STKEYS4, keys_x_offset, STBAR_Y_OFFSET + 13.1));
	} else if cards[Card::YellowCard as usize] {
		ui_to_render.push((Ui::STKEYS1, keys_x_offset, STBAR_Y_OFFSET + 14.0));
	}

	if cards[Card::RedSkull as usize] {
		ui_to_render.push((Ui::STKEYS5, keys_x_offset, STBAR_Y_OFFSET + 23.1));
	} else if cards[Card::RedCard as usize] {
		ui_to_render.push((Ui::STKEYS2, keys_x_offset, STBAR_Y_OFFSET + 24.0));
	}
}
...
pub fn update_arms_ui(weapon_owned: [bool; NUMWEAPONS], ui_to_render: &mut Vec<(Ui, f32, f32)>) {
	...
	ui_to_render.push((Ui::STARMS, 104.0, STBAR_Y_OFFSET));
	...
	ui_to_render.push((
		match (
			weapon_owned[WeaponType::Shotgun as usize],
			weapon_owned[WeaponType::SuperShotgun as usize],
		) {
...
fn big_red(mut n: i32, ui_to_render: &mut Vec<(Ui, f32, f32)>, num_x_offset: f32) -> f32 {
	let num_digits = if n < 10 {
		1
	} else if n < 100 {
		2
	} else {
