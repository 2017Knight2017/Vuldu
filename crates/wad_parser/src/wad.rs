...
impl Wad {
	pub fn open<P: AsRef<Path>>(path: P) -> Result<(Self, Vec<ParsedLump>, Vec<MapLumps>), String> {
		...
		if header != "IWAD" && header != "PWAD" {
			return Err(format!(
				"Invalid WAD format: {}, which is not IWAD or PWAD",
				header
			));
		}
		...
		while current_dir_pos < dir_end {
			...
			let offset = u32::from_le_bytes(
				data[current_dir_pos..current_dir_pos + 4]
					.try_into()
					.unwrap(),
			) as usize;
			let size = u32::from_le_bytes(
				data[current_dir_pos + 4..current_dir_pos + 8]
					.try_into()
					.unwrap(),
			) as usize;
			let name: [u8; 8] = data[current_dir_pos + 8..current_dir_pos + 16]
				.try_into()
				.unwrap();
