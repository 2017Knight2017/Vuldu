...
impl WadManager {
	...
	pub fn get_data(&self, lump_name: &[u8]) -> Result<&[u8], String> {
		...
		let lump = self.directory.get(&normalized_name).ok_or_else(|| {
			format!(
		...
		wad.data.get(start..end).ok_or_else(|| {
			format!(
				"Lump '{}' has out-of-bounds data slice",
				String::from_utf8_lossy(lump_name)
			)
		})
	}
	...
	pub fn get_colormap(&self, map_name: &[u8; 8]) -> Result<&[u8], String> {
		...
		let clm_data = self.wads[clm_wad_index]
			.data
			.get(clm_lump.offset..clm_lump.offset + clm_lump.size)
